...
def datawrite(summary, i):
    myFile = open('dataLh.csv', 'a', newline='')
    with myFile:
        fieldnames = ['summary']
...
for x in range(len(notesList)):
    ...
    # else:
    #     topsentence = rnLength
    summary_ln = summarizer_lh(parser.document, topsentence)
    print(type(summary_ln))



    for sen in summary_ln:
